...
export const deleteLayout = async (id) => {
  ...
  try {
    const response = await axios.delete(
      `${API_BASE_URL}/v1/layout/delete/${id}`,
      {
        headers: {
          authorization: token,
          session: channel,
...
export const updateProfile = async (id, jsonData) => {
  ...
  try {
    const response = await axios.put(
      `${API_BASE_URL}/v1/user/profile/update/${id}`,
      formData,
      {
        headers: {
...
export const isVerifyOTPScreenAvailable = async (screenName) => {
  ...
  const verify_otp_screen = screens.find(
    (screen) => screen.path === screenName
  );
...
export const getAllPaymentConfigs = async () => {
  try {
    const response = await axios.get(
      "https://xplr.live/api/v1/user/cashfree/getAllConfig",
      {
        headers: {
          Authorization: `Bearer ${token}`,
          session: channel,
...
export const assignPaymentConfigToCampaign = async (campaignId, configId) => {
  try {
    const response = await axios.post(
      `https://xplr.live/api/v1/user/cashfree/assign-to-campaign?campaignId=${campaignId}&configId=${configId}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const getAllWhatsAppConfigs = async () => {
    try {
        const response = await axios.get("https://xplr.live/api/v1/user/whatsapp/getAllConfig", {
            headers:{
                Authorization: `Bearer ${token}`,
                session:channel
            }
...
export const assignWhatsAppConfigToCampaign = async (campaignId, configId) => {
    try {
      const response = await axios.post(
        `https://xplr.live/api/v1/user/whatsapp/assign-to-campaign?campaignId=${campaignId}&configId=${configId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
