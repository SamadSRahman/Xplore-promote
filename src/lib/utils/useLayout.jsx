...
export default function useLayout() {
  ...
  const getAllLayout = async (id) => {
    ...
    try {
      const response = await axios.get(
        `${API_BASE_URL}/v1/layout/getAll/${id}`
      );
  ...
  const addDefaultLayouts = async (campaignId) => {
    await createLayout(
      JSON.stringify(splashScreenJSON),
      campaignId,
      "Splash Screen"
    );
    await createLayout(
      JSON.stringify(landingScreenJSON),
      campaignId,
      "Landing Screen",
      true
    );
    await createLayout(
      JSON.stringify(contactUsJSON),
      campaignId,
      "Contact Us Screen"
    );
  };
